...
import { TextInput, BooleanInput,  ReferenceArrayInput, ReferenceInput, SelectArrayInput, DateInput, SelectInput } from 'react-admin';
...
export const Listing = ({ table, fields }) => (props) =>
    <List {...props}>
        <Datagrid rowClick="edit">
            {fields.map((x, y) => {
                if (x.indexOf("fkOM") > -1 ){
                    console.log('x',x.slice(3, 4).toUpperCase() + x.slice(4, x.length).toLowerCase() + 's')
                    return  (                
                        <ReferenceField label={x.slice(5, x.length)}  source="id" reference={x.slice(5, 6).toUpperCase() + x.slice(6, x.length).toLowerCase() + 's'}>
                ...
                if (x.indexOf("url_") > -1) return <UrlField source={x} /> 
...
const displayEditOrCreate = (Component) => ({ table, fields }) => {
    ...
    return props => (
        <Component {...props}>
            <SimpleForm >
                {fields.map((x, y) => {
                    if (x.indexOf("fkOM_") > -1) {
                        return <ReferenceInput source={x} reference={x.slice(5, 6).toUpperCase() + x.slice(6, x.length).toLowerCase() + 's'}>
                            <SelectInput optionText="code" placeholder="hoho" />
                        </ReferenceInput>
                    }
                    if (x.indexOf("id") > -1) return <TextField source={x} /> 
                    ...
                    // if (x.indexOf("url_") > -1) return <UrlInput source={x} /> 
